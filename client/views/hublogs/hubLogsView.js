...
Template.hubLogsView.onCreated(function(){
    ...
    this.isValidForm = new ReactiveVar(false);
...
Template.hubLogsView.events({
    ...
    "keyup #rowsPerPage": (event, template) => {
        ...
        if(rowsPerPage === 0){
            event.target.style.borderColor = '';
            template.rowsPerPage.set(null);
        }
        else if(Number.isInteger(rowsPerPage) && rowsPerPage > 0){
            event.target.style.borderColor = '';
    ...
    "keyup #requestId": (event, template) => {
        ...
        if(reqId === 0){
            event.target.style.borderColor = '';
            template.requestId.set(null);
        }
        else if(Number.isInteger(reqId) && reqId > 0){
            event.target.style.borderColor = '';
    ...
    "keyup #serviceId": (event, template) => {
        ...
        if(serId === 0){
            event.target.style.borderColor = '';
            template.serviceId.set(null);
        }
        else if(Number.isInteger(serId) && serId > 0){
            event.target.style.borderColor = '';
    ...
    "keyup #severity": (event, template) => {
        ...
        if(sev === 'E' || sev === 'I' || sev === 'C' || sev === 'W' || sev === ''){
            event.target.style.borderColor = '';
    ...
    "keyup #noRecords": (event, template) => {
        ...
        if(recordSize === 0){
            event.target.style.borderColor = '';
            template.noRecords.set(null);
        }
        else if(Number.isInteger(recordSize) && recordSize > 0){
            event.target.style.borderColor = '';
    ...
    "change #environmentsDropDown": (event, template) => {
        event.preventDefault();
        template.selectedEnvironment.set({"env": $('#environmentsDropDown').val()});
    ...
    "click #btnSearch": (event, template) => {
        event.preventDefault();
        let params = {"env": $('#environmentsDropDown').val()};
...
function searchHubLogs(template){
    let params = {"env": getEnvironment()};
    if(template.noRecords.get() < 1){
        updateSearchParams('noRecords', DEFAULT_NO_RECORDS);
        template.noRecords.set(DEFAULT_NO_RECORDS);
    }
    if(template.rowsPerPage.get() < 1){
        updateSearchParams('rowsPerPage', DEFAULT_ROWS_PER_PAGE);
        template.rowsPerPage.set(DEFAULT_ROWS_PER_PAGE);
    }
...
function updateSearchUrl(params, queryParams){
    FlowRouter.go('hublogs', params, queryParams);
}
